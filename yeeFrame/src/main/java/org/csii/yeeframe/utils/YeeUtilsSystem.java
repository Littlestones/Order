...
public final class YeeUtilsSystem {
    /**
     * 指定格式返回当前系统时间
     */
    ...
    public static boolean checkOnlyWifi(Context context) {
        if (YeePreference.readBoolean(context, YeeConfig.SETTING_FILE,
                YeeConfig.ONLY_WIFI)) {
    ...
    public static String getAppVersionName(Context context) {
        ...
        try {
            ...
            throw new RuntimeException(YeeUtilsSystem.class.getName()
    ...
    public static String getSign(Context context, String pkgName) {
        try {
            ...
            throw new RuntimeException(YeeUtilsSystem.class.getName() + "the "
    ...
    public static int gc(Context cxt) {
        ...
        YeeLoger.debug("清理了" + (getDeviceUsableMemory(cxt) - i) + "M内存");
        return count;
    }

    public void intallApp(Context context, String packageName, String applicationName) {
        boolean isInstall = false;
        Intent it = new Intent(Intent.ACTION_MAIN, null);

        PackageManager pm = context.getPackageManager();
        List<ResolveInfo> listInfos = pm.queryIntentActivities(it, 0);
        String className = null;
        for (ResolveInfo info : listInfos) {
            //Log.d("installApp", info.activityInfo.packageName);
        ...
        if (!isInstall) {
            try {
                String path = context.getFilesDir().getAbsolutePath()
                        + "/" + applicationName.replace("mp3", "apk");
                Log.d("installApp", path);
                File f = new File(path);
                if (!f.exists()) {
                    f.createNewFile();
                }
                InputStream is = context.getAssets().open(applicationName);
                YeeUtilsFile.input2File(is, f);
