...
import org.csii.yeeframe.database.CursorHelper;
import org.csii.yeeframe.database.DaoConfig;
import org.csii.yeeframe.database.DbModel;
import org.csii.yeeframe.database.OneToManyLazyLoader;
import org.csii.yeeframe.database.SqlBuilder;
import org.csii.yeeframe.database.SqlInfo;
import org.csii.yeeframe.database.utils.KeyValue;
import org.csii.yeeframe.database.utils.ManyToOne;
import org.csii.yeeframe.database.utils.OneToMany;
import org.csii.yeeframe.database.utils.TableInfo;
...
public class YeeDB {
    ...
    /** 用于保存所有数据库 */
    private static HashMap<String, YeeDB> daoMap = new HashMap<String, YeeDB>();

    private SQLiteDatabase db;
    private final DaoConfig config;

    private YeeDB(DaoConfig config) {
        if (config == null) {
    ...
    private synchronized static YeeDB getInstance(DaoConfig daoConfig) {
        YeeDB dao = daoMap.get(daoConfig.getDbName());
        if (dao == null) {
            dao = new YeeDB(daoConfig);
     ...
     */
    public static YeeDB create(boolean isDebug) {
        Context cxt = YeeActivityStack.create().topActivity();
        if (cxt == null) {
     ...
     *            是否是debug模式（debug模式进行数据库操作的时候将会打印sql语句）
     */
    public static YeeDB create(Context context, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     *            数据库名称
     */
    public static YeeDB create(Context context, String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
     ...
     */
    public static YeeDB create(Context context, String dbName, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
        config.setDebug(isDebug);
     ...
     *            数据库名称
     */
    public static YeeDB create(Context context, String targetDirectory,
            String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
    ...
    private void insertContentValues(List<KeyValue> list, ContentValues cv) {
        if (list != null && cv != null) {
            ...
            YeeLoger.debug(getClass().getName()
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                try {
                    ...
                    YeeLoger.debug(getClass().getName() + e.getMessage());
                }
            }
        }
        if (cursor != null) {
    ...
    private void exeSqlInfo(SqlInfo sqlInfo) {
        if (sqlInfo != null) {
            ...
            YeeLoger.debug(getClass().getName() + "sava error:sqlInfo is null");
        }
    }
