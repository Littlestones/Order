...
public abstract class FrameActivity extends FragmentActivity implements
        OnClickListener, IBroadcastReg, IYeeActivity, ISkipActivity {

    public static final int WHICH_MSG = 0X37210;

    protected YeeFragment currentYeeFragment;
     ...
     */
    public void changeFragment(int resView, YeeFragment targetFragment) {
        if (targetFragment.equals(currentYeeFragment)) {
            return;
        }
        ...
        if (currentYeeFragment != null && currentYeeFragment.isVisible()) {
            transaction.hide(currentYeeFragment);
        }
        currentYeeFragment = targetFragment;
        transaction.commit();
    }
