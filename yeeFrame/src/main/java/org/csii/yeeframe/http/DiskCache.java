/*
 * Copyright (c) 2014, Android Open Source Project,CSII.
 *
...
public class DiskCache implements Cache {
    ...
    public synchronized void clean() {
        ...
        mEntries.clear();
        mTotalSize = 0;
        YeeLoger.debug("disk Cache cleared.");
    }
    ...
    public synchronized Entry get(String key) {
        ...
        try {
            ...
            YeeLoger.debug("%s: %s", file.getAbsolutePath(), e.toString());
            remove(key);
            return null;
        } finally {
            if (cis != null) {
    ...
    public synchronized void initialize() {
        if (!mRootDirectory.exists()) {
            if (!mRootDirectory.mkdirs()) {
                YeeLoger.debug("Unable to create cache dir %s",
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        try {
            ...
            if (!success) {
                fos.close();
                YeeLoger.debug("Failed to write header for %s",
        ...
        if (!deleted) {
            YeeLoger.debug("Could not clean up file %s", file.getAbsolutePath());
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        if (HttpConfig.DEBUG) {
            YeeLoger.debug("Pruning old cache entries.");
        }
        ...
        if (HttpConfig.DEBUG) {
            YeeLoger.debug("pruned %d files, %d bytes, %d ms", prunedFiles,
