...
import org.csii.yeeframe.http.YeeHttpException;
...
public class ImageDisplayer {
    private final YeeHttp mYeeHttp; // 使用YeeHttp的线程池执行队列去加载图片
    ...
    public ImageBale get(String requestUrl, int maxWidth, int maxHeight,
        ...
        newRequest.setConfig(mYeeHttp.getConfig());
        mYeeHttp.doRequest(newRequest);
        ...
        return new ImageRequest(requestUrl, maxWidth, maxHeight,
                new HttpCallBack() {
                    ...
                    public void onFailure(int errorNo, String strMsg) {
                        ...
                        onGetImageError(requestUrl, new YeeHttpException(strMsg));
                    }
                });
    }
     ...
     *            失败原因
     */
    protected void onGetImageError(String url, YeeHttpException error) {
        // 从正在请求的列表中移除这个已完成的请求
        ImageRequestEven request = mRequestsMap.remove(url);
        if (request != null) {
    ...
    private class ImageRequestEven {
        ...
        private YeeHttpException mError;
        ...
        public void setError(YeeHttpException error) {
            mError = error;
        }

        public YeeHttpException getError() {
            return mError;
        }
