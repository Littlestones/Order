...
import org.csii.yeeframe.http.Cache;
import org.csii.yeeframe.http.CacheDispatcher;
import org.csii.yeeframe.http.Delivery;
import org.csii.yeeframe.http.DownloadController;
import org.csii.yeeframe.http.DownloadTaskQueue;
import org.csii.yeeframe.http.FileRequest;
import org.csii.yeeframe.http.FormRequest;
...
import org.csii.yeeframe.http.JsonRequest;
import org.csii.yeeframe.http.NetworkDispatcher;
import org.csii.yeeframe.http.Request;
import org.csii.yeeframe.http.Request.HttpMethod;
 ...
 * 
 * Note:<br>
 * 整个YeeHttp工作流程：采用责任链设计模式，由三部分组成，类似设计可以类比Handle...Looper...MessageQueue<br>
 * 
 * 1、YeeHttp负责不停向NetworkQueue(或CacheQueue实际还是NetworkQueue， 具体逻辑请查看
...
public class YeeHttp {
    ...
    public YeeHttp(HttpConfig config) {
        this.mConfig = config;
     ...
     *            哪条url的缓存
     * @param params
     *            http请求中的参数集(YeeHttp的缓存会连同请求参数一起作为一个缓存的key)
     * @since 2.234以后有用
     */
    ...
    /**
     * 向请求队列加入一个请求<br>
     * Note:此处工作模式是这样的：YeeHttp可以看做是一个队列类，而本方法不断的向这个队列添加request；另一方面，
